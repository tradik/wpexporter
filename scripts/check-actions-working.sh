...
# Working version of GitHub Actions version checker
# Usage: ./scripts/check-actions-working.sh
...
get_current_version() {
    local repo="$1"
    local result
    result=$(grep -h "uses.*$repo@" .github/workflows/*.yml | head -1 | sed 's/^[[:space:]]*//' | cut -d'@' -f2 || echo "unknown")
    echo "${result:-unknown}"
}
...
# Create report
cat > docs/github-actions-versions-working.md << EOF
...
for action in "${actions[@]}"; do
    ...
    echo "$action → $status"
    echo "| $action | $current | $latest | $status |" >> docs/github-actions-versions-working.md
done

echo ""
echo -e "${GREEN}✅ Report saved to: docs/github-actions-versions-working.md${NC}"
echo ""
