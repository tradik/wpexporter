...
| Tool | Version | Where |
|------|---------|-------|
| Go | 1.27.1 | `go-version:` in every job. `go.mod` and `tools/go.mod` stay at the **minor floor** `go 1.27.0`: the `go` directive is a minimum, not a pin, and pinning a patch there once broke the snap build |
