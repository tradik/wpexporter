...
<tr><td><code>--from-sitemap</code></td><td>When the REST API serves no posts, recover what the site's feed still publishes (title, address, date, author, body — no IDs, terms or featured images). Turns itself on for a WordPress older than 4.7, which has no content API to read</td><td><code>false</code></td></tr>
