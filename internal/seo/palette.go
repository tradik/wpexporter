...
//
// A migration carries the content and loses the look. The colors, though, are
...
// that arrives in its own colors and one that arrives in the generator's
// defaults with a note to "rebuild the theme".
//
// Only declarations inside <style> are read, and only values that are literally
// colors: the palette travels to another system's stylesheet, so a var()
...
// paletteNamedColors is the handful of CSS color keywords a theme variable is
// realistically set to. It only has to RECOGNIZE a color — the contrast
...
// real color wins, so a theme's own variable beats a page builder's default —
...
// <style> blocks whose value is a literal color. The first declaration wins:
...
// normalizeColor returns the value when it is a literal CSS color, or "" when
// it is anything else — a var() reference, a gradient, a length, a font stack.
func normalizeColor(raw string) string {
	value := strings.TrimSpace(raw)
	// A trailing "!important" is a stylesheet concern, not part of the color.
